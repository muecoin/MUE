...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"phore-address\" monthly-payment\n"
...
UniValue mnfinalbudget(const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "show") {
        ...
        for (CFinalizedBudget* finalizedBudget : winningFbs)
        {
            ...
            if (finalizedBudget->GetBlockStart() < (chainActive.Tip()->nHeight - Params().GetBudgetCycleBlocks())) continue;
